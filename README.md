...
This is as a result of experiencing **terrible performance** and **ZombieCheck()** issues when using the default implementation of Entity Framework Core with a Microsoft SQL server.

**TLDR:**  If you are planning to use Entity Framework Core with Microsoft SQL Server in a Production environment it is highly recommended that you consider changing some of the default values. Either by disabling auto transactions in your Entity Framework solution or by chaining your lock isolation behavior in code or the database.
...
- Basic API to insert records into a MS SQL database using 3 different techniques.
  - Inserting using EF Core with a table that has an auto increment primary key.
  - Inserting using EF Core with a table that has a Guid primary key.
  - Inserting using a Stored Procedure that inserts into a table that has an auto increment primary key.
...
Simple API that inserts records into a MS SQL database using EF Core. The table has an **auto increment Id** as a **Primary Ke**y.
...
Simple API that inserts records into a MS SQL database using EF Core. The table has an **auto increment Id** as a **Primary Key**. Based on the results above and as suggested by Microsoft the **EnableRetryOnFailure** execution policy was added.
...
Simple API that inserts records into a MS SQL database using EF Core. The table has an **auto increment Id** as a **Primary Key**. Based on the results above and online research it was decided to test with a **Custom Execution Strategy**.
...
Simple API that inserts records into a MS SQL database using EF Core. The table has an **auto increment Id** as a **Primary Key**.
...
This can be avoided in 3 ways:

1. Running SaveChanges in a ReadUncommitted Transaction (Would have to be applied per transaction or Database interaction).
...
2. Setting AutoTransactionsEnabled to false (This can be done on a global context level or per transaction but means transactions won't be created automatically).

    ```c#
    _context.Database.AutoTransactionsEnabled = false;
    ```

3. Enabling Read Committed Snapshot on the Database (Database change). This is a recommendation from [Scaling .Net Core Applications](https://www.carlrippon.com/scalable-and-performant-asp-net-core-web-apis-sql-server-isolation-level/) for general performance that could assist on reading from the database as well.
...
Simple API that inserts records into a MS SQL database using EF Core. The table has a **Guid Id** as a **Primary Key**. Based on the deadlock issue above it was decided to test with a Guid Id that is generated in code thus not creating a deadlock.
...
Simple API that inserts records into a MS SQL database using EF Core. The table has an a**uto increment Id** as a **Primary Key**. Based on the deadlock issue above it was decided to test the insert with a **Stored Procedure** hoping it would handle the deadlock issue better.
...
Simple API that inserts records into a MySql database using EF Core (Pomelo.EntityFrameworkCore.MySql). The table has an **auto increment Id** as a **Primary Key**.
...
If a table with an **Auto Incremented Id** is used the load that the default implementation of EF Core and MS SQL can handle is **extremely disappointing**.It isn't close to the load that can be handled by a Stored Procedure unless the d**efault transaction isolation levels are changed**.

An alternative would be to use another form of Primary Key like a Guid, however this introduces its own challenges.
